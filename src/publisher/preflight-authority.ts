...
export type AuthorizedArtifact = { id: string; name: string; version: string; kind: "npm" | "cargo"; path: string; sha256: Sha256; size: number; ino: number; mode: 256; cargoMetadata: JsonValue | null; cargoMetadataSha256: Sha256 | null };
  ...
  async consume(proof: { proofId: string; bindingDigest: string; issuedAt: string; expiresAt: string; token: string }, facts: { eventId: string; nonce: string; planId: string; releaseCommit: string; ref: string }, artifacts: AuthorizedArtifact[], authenticate: () => Promise<boolean>): Promise<{ accepted: true; eventId: string; proofId: string; authorization: PublishAuthorization; signature: string }> {
    ...
    return transact(this.store, (snapshot) => {
      ...
      for (const [index, artifact] of artifacts.entries()) {
        const selected = (stored.binding.packages as Array<{ id: string; version: string }>)[index]; const expectedName = artifact.id.startsWith("npm:@lenso/") ? artifact.id.slice("npm:@lenso/".length) : artifact.id.slice("cargo:".length);
        if (!selected || artifact.id !== selected.id || artifact.version !== selected.version || artifact.name !== expectedName || artifact.kind !== (artifact.id.startsWith("npm:") ? "npm" : "cargo") || !artifact.path.startsWith(`.lenso-release/preflight-artifacts/${proof.proofId.slice(7)}/`) || !/^sha256:[0-9a-f]{64}$/u.test(artifact.sha256) || !Number.isSafeInteger(artifact.size) || artifact.size <= 0 || !Number.isSafeInteger(artifact.ino) || artifact.ino <= 0 || artifact.mode !== 0o400) throw new Error("invalid canonical artifact authorization");
