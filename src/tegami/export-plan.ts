...
function copiesCargoWorkspaceSource(cwd: string, source: string): boolean {
...
async function assertSupportedCargoSources(cwd: string): Promise<void> {
  try { await lstat(join(cwd, "Cargo.toml")); } catch (error) {
...
async function assertSafeWorkspacePath(cwd: string, path: string, allowMissing: boolean): Promise<void> {
  const relativePath = relative(resolve(cwd), resolve(path));
  if (!relativePath || relativePath.startsWith("..") || relativePath.includes("\\")) fail(`unsafe workspace path ${path}`);
  ...
  for (const [index, segment] of segments.entries()) {
    ...
    try {
      ...
      if (status.isSymbolicLink()) fail(`unsafe workspace symlink ${relativePath}`);
      if (index < segments.length - 1 && !status.isDirectory()) fail(`unsafe workspace parent ${relativePath}`);
      if (index === segments.length - 1 && !status.isFile()) fail(`unsafe workspace target ${relativePath}`);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === "ENOENT" && allowMissing && index === segments.length - 1) return;
      throw error;
    }
  }
}
...
async function snapshotWorkspace(cwd: string, packages: Iterable<WorkspacePackage>): Promise<Snapshot> {
  ...
  try {
    ...
    if (!tegamiDirectory.isDirectory() || tegamiDirectory.isSymbolicLink()) fail("unsafe workspace .tegami directory");
    for (const entry of await readdir(join(cwd, ".tegami"), { withFileTypes: true })) {
      if (entry.name.endsWith(".md")) {
        if (!entry.isFile() || entry.isSymbolicLink()) fail(`unsafe Tegami intent ${entry.name}`);
  ...
  for (const path of paths) {
    ...
    try { snapshot.set(path, await safeRead(cwd, path)); }
    catch (error) {
...
async function restoreSnapshot(cwd: string, snapshot: Snapshot): Promise<void> {
  for (const [path, content] of snapshot) {
    await assertSafeWorkspacePath(cwd, path, true);
    if (content === undefined) {
      try {
        const status = await lstat(path);
        if (!status.isFile() || status.isSymbolicLink()) fail(`unsafe rollback target ${relative(cwd, path)}`);
        await rm(path);
      } catch (error) {
        if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
      }
...
async function restoreIgnoredPackageFiles(
  options: ExportReleasePlanOptions,
  packages: ReadonlyMap<string, WorkspacePackage>,
  snapshot: Snapshot,
): Promise<void> {
  const ignored = new Set(options.ignore ?? []);
  const restore = new Map<string, Buffer | undefined>();
  for (const pkg of packages.values()) {
    if (!ignored.has(pkg.id) && !ignored.has(pkg.name)) continue;
    for (const path of [
      join(pkg.path, pkg.manager === "cargo" ? "Cargo.toml" : "package.json"),
      join(pkg.path, "CHANGELOG.md"),
    ]) {
      if (snapshot.has(path)) restore.set(path, snapshot.get(path));
    }
  }
  await restoreSnapshot(options.cwd, restore);
}

async function preserveHistoricalChangelogs(
  options: ExportReleasePlanOptions,
  ...
  for (const path of paths) {
    ...
    if (after.indexOf(before) !== -1) continue;
...
function buildPlan(
  options: ExportReleasePlanOptions, releasePackages: ReleasePackage[],
  generatedFiles: ReleasePlanV1["generatedFiles"],
): ReleasePlanV1 {
  const identity = {
    schema: "lenso.release-plan.v1" as const, repository: options.repository, sourceCommit: options.sourceCommit,
    tegamiVersion: "1.2.5" as const, publisher: options.publisher, generatedFiles, packages: releasePackages,
  };
...
async function collectGeneratedFiles(
  cwd: string, paths: string[],
): Promise<ReleasePlanV1["generatedFiles"]> {
  return Promise.all(paths.map(async (path) => ({
    path,
    sha256: sha256(await safeRead(cwd, join(cwd, path))) as Sha256,
  })));
}
