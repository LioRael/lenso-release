 ...
 */
export async function repairVirtualWorkspaceManifest(cwd: string): Promise<void> {
  ...
  if (!/^\[workspace\]$/mu.test(original) || /^\[package\]$/mu.test(original)) return;
  const lines = original.split("\n");
  const start = lines.findIndex((line) => line === "[dependencies]");
  if (start === -1) return;
  let end = start + 1;
  while (end < lines.length && (!lines[end]!.startsWith("[") || lines[end]!.startsWith("[dependencies."))) end += 1;
  const generated = lines.slice(start, end).join("\n");
  const versions = [...generated.matchAll(/^\[dependencies\.([^\]]+)\]\nversion = "([^"]+)"$/gmu)];
  if (versions.length === 0) throw new Error("virtual workspace dependency repair found no exact versions");
  ...
  for (const match of versions) {
    ...
    if (!dependency.test(repaired)) throw new Error(`virtual workspace dependency ${key} has no versioned workspace entry`);
    repaired = repaired.replace(dependency, `$1${version}$2`);
  }
  await writeFile(path, repaired);
}

export function repairCargoWorkspace(): TegamiPlugin {
  return {
