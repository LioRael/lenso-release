...
export const RETRIED_PRODUCTION_ZERO_WRITE_RECOVERY =
  "retried production zero-write recovery";
...
export function assertLegalTransition(
  ...
  const partialRecoveryAttempt = appendedAttempts.find(({ kind, outcome, detail }) =>
    ...
    (detail === AUTHORIZED_PRODUCTION_PARTIAL_RECOVERY ||
      ...
      detail === RETRIED_PRODUCTION_ZERO_WRITE_RECOVERY)
  );
...
export async function supersedeFailedProductionZeroWriteRecovery(
  ...
  if (mode !== "production")
    throw new Error("zero-write recovery retry is restricted to production mode");
  if (
    !/^[A-Za-z0-9](?:[A-Za-z0-9._/-]{0,254})$/u.test(workflowRef) ||
    workflowRef.includes("..") ||
    !OID.test(workflowCommit)
  )
    throw new TypeError("zero-write recovery retry workflow ref invalid");
  const runUrl =
    /^https:\/\/github\.com\/[A-Za-z0-9._-]+\/[A-Za-z0-9._-]+\/actions\/runs\/[1-9][0-9]*$/u;
  if (!runUrl.test(failedRunUrl) || !runUrl.test(proofRunUrl))
    throw new TypeError("zero-write recovery retry evidence URL invalid");
  const initial = await store.readSnapshot();
  assertReleaseStateSnapshot(initial);
  const path = planStatePath(repository, planId);
  const state = initial.plans[path];
  ...
  if (
    ...
    previous.recovery?.kind !== "production-zero-write"
  )
    throw new Error("zero-write recovery dispatch is not supersedable");
  if (previous.recovery.failedRunUrl !== failedRunUrl)
    throw new Error("zero-write recovery retry original publisher mismatch");
  if (previous.recovery.workflowCommit === workflowCommit)
    throw new Error("zero-write recovery workflow identity did not change");
  const observed = await facts.observeRun(previous);
  if (
    !observed ||
    observed.runUrl !== previous.runUrl ||
    observed.status !== "completed" ||
    !["failure", "cancelled"].includes(observed.conclusion ?? "")
  )
    throw new Error("zero-write recovery workflow is not conclusively failed");
  const original = state.outbox.filter(
    ({ recovery, runUrl: url }) => recovery === undefined && url === failedRunUrl,
  );
  if (original.length !== 1)
    throw new Error("zero-write recovery retry requires the exact original publisher");
  ...
  if (JSON.stringify(selected) !== JSON.stringify(dispatched))
    throw new Error("zero-write recovery retry does not cover the active slice");
  const at = now.toISOString();
  ...
  const committed = await transact(store, (current) => {
    ...
    if (!candidate || candidate.revision !== state.revision)
      throw new StateConflictError(
        "plan changed while failed zero-write recovery was observed",
      );
    const result: PlanStateV1 = {
      ...candidate,
