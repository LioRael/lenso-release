...
import type { OciReleaseArtifact } from "./oci-release-artifact.js";
...
function registryBase(value: string): URL {
  ...
  if (url.protocol !== "https:" || url.username || url.password || url.search || url.hash) fail("registry endpoint is invalid");
  return url;
}
function challenge(value: string): { realm: string; service: string; scope: string } {
  if (!value.startsWith("Bearer ")) fail("registry authentication challenge is invalid");
  const fields = Object.fromEntries([...value.slice(7).matchAll(/([a-z]+)="([^"]+)"/gu)].map((match) => [match[1], match[2]]));
  if (!fields.realm || !fields.service || !fields.scope) fail("registry authentication challenge is incomplete");
  return fields as { realm: string; service: string; scope: string };
}
...
export async function publishOciImage(input: {
...
}): Promise<{ digest: Sha256; manifestBytes: Buffer; registryUrl: string; publishedAt: string }> {
  const base = registryBase(input.registry); const prefix = base.pathname.replace(/\/+$/u, ""); const request = input.fetch ?? globalThis.fetch;
  ...
  const endpoint = (path: string) => new URL(`${prefix}/v2/${input.artifact.registryRepository}/${path}`, base.origin);
  const perform = async (url: URL, init: RequestInit = {}): Promise<Response> => {
    ...
    const parsed = challenge(response.headers.get("www-authenticate") ?? ""); const realm = new URL(parsed.realm);
    if (realm.protocol !== "https:" || realm.origin !== base.origin) fail("registry token realm is not trusted");
    realm.searchParams.set("service", parsed.service); realm.searchParams.set("scope", parsed.scope);
    ...
    let body: JsonObject = {}; try { body = await tokenResponse.json() as JsonObject; } catch { /* handled below */ }
    const token = body.token ?? body.access_token;
    if (!tokenResponse.ok || typeof token !== "string") fail(`registry token exchange failed with ${tokenResponse.status}`);
  ...
  for (const [digest, bytes] of input.artifact.blobs) {
    if (digest === input.artifact.manifestDigest) continue;
    const existing = await perform(endpoint(`blobs/${digest}`), { method: "HEAD" });
    if (existing.ok) continue;
    if (existing.status !== 404) fail(`blob observation failed with ${existing.status}`);
  ...
  const stored = await perform(manifestUrl, { method: "PUT", headers: { "content-type": MANIFEST_TYPE }, body: input.artifact.manifestBytes as unknown as BodyInit });
  if (stored.status !== 201 || stored.headers.get("docker-content-digest") !== input.artifact.manifestDigest) fail(`manifest upload failed with ${stored.status}`);
  const observed = await perform(manifestUrl, { headers: { accept: MANIFEST_TYPE } });
  if (!observed.ok || observed.headers.get("docker-content-digest") !== input.artifact.manifestDigest) fail(`manifest observation failed with ${observed.status}`);
  const manifestBytes = Buffer.from(await observed.arrayBuffer());
  if (!manifestBytes.equals(input.artifact.manifestBytes)) fail("remote manifest bytes contradict the sealed image");
