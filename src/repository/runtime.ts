...
type RepositoryConfig = {
  schema: "lenso.repository-config.v1";
  repository: string;
...
type PreflightProof = {
  schema: "lenso.publisher-preflight-proof.v1";
  proofId: Sha256;
...
type CargoBootstrapPolicy = { schema: "lenso.cargo-bootstrap.v1"; packages: PublishSelection[] };
type CargoBootstrapRecoveryPolicy = Omit<CargoBootstrapPolicy, "schema"> & { schema: "lenso.cargo-bootstrap-recovery.v1"; planId: string; releaseCommit: string };
async function committedJson(environment: RuntimeEnvironment, commit: string, path: string): Promise<unknown | null> {
  const tracked = (await execFile("git", ["ls-tree", "-r", "--name-only", commit, "--", path], { cwd: environment.cwd })).stdout.trim();
  if (!tracked) return null;
  try { return JSON.parse((await execFile("git", ["show", `${commit}:${path}`], { cwd: environment.cwd })).stdout); }
  catch { fail(`Cargo bootstrap policy is unreadable from reviewed commit ${commit}`); }
}
function cargoBootstrapSelections(parsed: unknown, schema: CargoBootstrapPolicy["schema"] | CargoBootstrapRecoveryPolicy["schema"]): Set<string> {
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) fail("Cargo bootstrap policy must be an object");
  const policy = parsed as Partial<CargoBootstrapPolicy>;
  if (policy.schema !== schema || !Array.isArray(policy.packages)) fail("Cargo bootstrap policy schema is invalid");
  const seen = new Set<string>();
  for (const selected of policy.packages) {
    if (!selected || typeof selected !== "object" || typeof selected.id !== "string" || typeof selected.version !== "string") fail("Cargo bootstrap package selection is invalid");
    if (!selected.id.startsWith("cargo:") || !PACKAGE.test(selected.id) || !VERSION.test(selected.version)) fail("Cargo bootstrap package identity is invalid");
    const identity = `${selected.id}@${selected.version}`;
    if (seen.has(identity)) fail("Cargo bootstrap package selection is duplicated");
    seen.add(identity);
  }
  return seen;
}
export async function cargoRegistryTokenFor(environment: RuntimeEnvironment, item: PublishSelection): Promise<string> {
  const trustedPublishingToken = process.env.CARGO_REGISTRY_TOKEN;
  if (!trustedPublishingToken) fail("official crates.io token is required without fallback");
  if (process.env.LENSO_RELEASE_MODE !== "production") return trustedPublishingToken;
  ...
  if (
    !selected &&
    ["production-zero-write", "production-partial"].includes(
      process.env.LENSO_CARGO_BOOTSTRAP_RECOVERY ?? "",
    )
  ) {
    const parsed = await committedJson(environment, environment.githubSha, ".lenso-release/cargo-bootstrap-recovery.json");
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) fail("Cargo bootstrap recovery policy is missing");
    const recovery = parsed as Partial<CargoBootstrapRecoveryPolicy>;
    if (recovery.planId !== environment.planId || recovery.releaseCommit !== environment.releaseCommit) fail("Cargo bootstrap recovery policy binding is invalid");
    selected = cargoBootstrapSelections(parsed, "lenso.cargo-bootstrap-recovery.v1").has(identity);
  }
  if (!selected) return trustedPublishingToken;
  const bootstrapToken = process.env.LENSO_CARGO_BOOTSTRAP_TOKEN;
  if (!bootstrapToken) fail(`Cargo bootstrap token is required for ${item.id}@${item.version}`);
...
async function reviewedRegistryBindings(cwd: string, plan: ReleasePlanV1): Promise<Record<string, string>> {
  ...
  for (const item of plan.packages) {
    ...
    if (item.id.startsWith("oci:")) {
      ...
      if (!component.registryPath || configured !== component.registryPath) fail(`unreviewed OCI registry destination: ${item.id}`);
...
export async function preflight(environment: RuntimeEnvironment): Promise<ReleasePlanV1> {
  if (process.env.LENSO_RELEASE_MODE !== "shadow" && process.env.LENSO_RELEASE_MODE !== "production") fail("LENSO_RELEASE_MODE must be shadow or production");
  if (!/^sha256:[0-9a-f]{64}$/u.test(environment.eventId) || !/^[0-9a-f-]{16,64}$/u.test(environment.nonce)) fail("invalid event ID or nonce");
...
async function gateBinding(environment: RuntimeEnvironment): Promise<{ plan: ReleasePlanV1; binding: Record<string, JsonValue>; digest: Sha256 }> {
  ...
  const binding: Record<string, JsonValue> = {
    ...
    packages: environment.packages as unknown as JsonValue, generated: generated as unknown as JsonValue,
    registries: await reviewedRegistryBindings(environment.cwd, plan) as unknown as JsonValue,
  };
  return { plan, binding, digest: sha256(binding) as Sha256 };
}
async function writeProof(cwd: string, proof: PreflightProof): Promise<void> {
  const directory = join(cwd, ".lenso-release"); await mkdir(directory, { recursive: false }).catch((error: NodeJS.ErrnoException) => { if (error.code !== "EEXIST") throw error; });
  const target = join(directory, "preflight-proof.json"); const temporary = join(directory, `.preflight-proof-${crypto.randomUUID()}.tmp`);
  const handle = await open(temporary, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY | constants.O_NOFOLLOW, 0o600);
  try { await handle.writeFile(Buffer.concat([canonicalBytes(proof as unknown as JsonValue), Buffer.from("\n")])); await handle.sync(); } finally { await handle.close(); }
  await rename(temporary, target);
}
export async function createPreflightProof(environment: RuntimeEnvironment): Promise<PreflightProof> {
...
export async function consumePreflightProof(environment: RuntimeEnvironment): Promise<SealedMarker> {
  const { digest } = await gateBinding(environment);
  let proofBytes: Buffer; try { proofBytes = await safeRead(environment.cwd, ".lenso-release/preflight-proof.json"); } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") fail("preflight proof is missing or already consumed"); throw error; }
  const proof = parseJson<PreflightProof>(proofBytes, "preflight proof");
  ...
  const confirmation = await response.json() as { accepted?: boolean; eventId?: string; proofId?: string; authorization?: PublishAuthorization; signature?: string };
  if (confirmation.accepted !== true || confirmation.eventId !== environment.eventId || confirmation.proofId !== proof.proofId || !confirmation.authorization || typeof confirmation.signature !== "string") fail("coordinator preflight proof was not atomically consumed");
...
function verifyAuthorization(authorization: PublishAuthorization, signature: string, digest: Sha256, environment: RuntimeEnvironment, artifacts: AuthorizedArtifact[]): void {
  const publicKey = process.env.LENSO_PREFLIGHT_AUTHORITY_PUBLIC_KEY; if (!publicKey) fail("preflight authority public key is required");
  if (authorization.schema !== "lenso.publisher-authorization.v1" || authorization.bindingDigest !== digest || authorization.eventId !== environment.eventId || authorization.nonce !== environment.nonce || authorization.planId !== environment.planId || authorization.releaseCommit !== environment.releaseCommit || authorization.ref !== environment.refName || Date.parse(authorization.expiresAt) <= Date.now() || !canonicalBytes(authorization.artifacts as unknown as JsonValue).equals(canonicalBytes(artifacts as unknown as JsonValue)) || !verifySignature(null, canonicalBytes(authorization as unknown as JsonValue), createPublicKey(publicKey), Buffer.from(signature, "base64url"))) fail("server publish authorization signature is invalid");
}
async function cargoWireMetadataFromCrate(cratePath: string, name: string, version: string): Promise<JsonValue> {
  const directory = await mkdtemp(join(tmpdir(), "lenso-crate-metadata-")); try {
    await execFile("tar", ["-xzf", cratePath, "-C", directory]); const roots = await readdir(directory); if (roots.length !== 1 || roots[0] !== `${name}-${version}`) fail("Cargo archive root identity mismatch");
    const manifest = join(directory, roots[0], "Cargo.toml"); const metadata = JSON.parse((await execFile("cargo", ["metadata", "--manifest-path", manifest, "--no-deps", "--format-version", "1"])).stdout) as { packages?: Array<Record<string, unknown>> };
    const pkg = metadata.packages?.find((entry) => entry.name === name && entry.version === version); if (!pkg) fail("Cargo sealed manifest identity mismatch"); return cargoWireMetadata(pkg, name, version);
  } finally { await rm(directory, { recursive: true, force: true }); }
}
function cargoWireMetadata(pkg: Record<string, unknown>, name: string, version: string): JsonValue {
  return { name, vers: version, deps: pkg.dependencies as JsonValue ?? [], features: pkg.features as JsonValue ?? {}, authors: pkg.authors as JsonValue ?? [], description: pkg.description as JsonValue ?? null, documentation: pkg.documentation as JsonValue ?? null, homepage: pkg.homepage as JsonValue ?? null, readme: pkg.readme as JsonValue ?? null, keywords: pkg.keywords as JsonValue ?? [], categories: pkg.categories as JsonValue ?? [], license: pkg.license as JsonValue ?? null, license_file: pkg.license_file as JsonValue ?? null, repository: pkg.repository as JsonValue ?? null, badges: {}, links: pkg.links as JsonValue ?? null, rust_version: pkg.rust_version as JsonValue ?? null };
}
...
async function npmObservation(name: string, version: string): Promise<RegistryObservation> {
  ...
  const metadata = (body.versions as Record<string, Record<string, unknown>> | undefined)?.[version];
  ...
  const publishedAt = String((body.time as Record<string, unknown> | undefined)?.[version] ?? "");
  ...
  if (process.env.LENSO_TEST_ARTIFACT_PROXY_URL && process.env.NODE_ENV !== "test") fail("artifact proxy is test-only");
...
async function artifactObservation(name: string, version: string, environment: RuntimeEnvironment): Promise<RegistryObservation> {
  ...
  if (release.status === 404) return { exists: false };
  if (!release.ok) fail(`hosted artifact release observation ${release.status}`);
  const body = await release.json() as { draft?: boolean; created_at?: string; assets?: Array<{ name?: string; url?: string; browser_download_url?: string }> };
  if (body.draft !== true || !body.created_at) fail("hosted artifact release must remain a verified draft");
  ...
  if (!asset || !checksumAsset) return { exists: false };
  if (!asset.url || !asset.browser_download_url || !checksumAsset.url) fail("hosted artifact release asset is incomplete");
  const download = await fetchGithubReleaseAsset(asset.url, api, { ...headers, accept: "application/octet-stream" });
  if (!download.ok) fail(`hosted artifact download ${download.status}`);
  const bytes = new Uint8Array(await download.arrayBuffer());
  const checksum = await fetchGithubReleaseAsset(checksumAsset.url, api, { ...headers, accept: "application/octet-stream" });
  if (!checksum.ok) fail(`hosted artifact checksum download ${checksum.status}`);
...
async function packedArtifact(cwd: string, item: PublishSelection): Promise<PackedArtifact> {
  ...
  if (item.id.startsWith("artifact:")) {
    ...
    if (!artifact) fail(`hosted artifact configuration is missing: ${item.id}`);
...
async function publishOnce(environment: RuntimeEnvironment, item: PublishSelection, artifact: SealedArtifact): Promise<void> {
  if (item.id.startsWith("npm:")) {
    ...
    if (process.env.CARGO_TOKEN) fail("Cargo token fallback is forbidden");
    if (!artifact.cargoMetadata) fail("signed Cargo upload metadata missing"); await uploadCargoArtifact(item, artifact.bytes, artifact.cargoMetadata, await cargoRegistryTokenFor(environment, item));
    ...
    if (!releaseResponse.ok) fail(`draft hosted artifact release creation ${releaseResponse.status}`);
    const release = await releaseResponse.json() as { draft?: boolean; target_commitish?: string; upload_url?: string; assets?: Array<{ name?: string; url?: string }> };
    if (release.draft !== true || release.target_commitish !== environment.releaseCommit) fail("hosted artifact draft identity mismatch");
    const uploadBase = release.upload_url?.replace(/\{.*$/u, "");
    if (!uploadBase) fail("draft hosted artifact upload URL is missing");
    ...
    const ensureAsset = async (name: string, bytes: Uint8Array, contentType: string): Promise<void> => {
      ...
      if (existing?.url) {
        ...
        if (!downloaded.ok || !Buffer.from(await downloaded.arrayBuffer()).equals(Buffer.from(bytes))) fail(`draft hosted artifact asset contradicts sealed bytes: ${name}`);
        return;
      }
      const response = await upload(name, bytes, contentType);
      if (!response.ok) fail(`draft hosted artifact upload ${response.status}: ${name}`);
    };
...
export async function uploadCargoArtifact(item: PublishSelection, bytes: Buffer, upload: JsonValue, token = process.env.CARGO_REGISTRY_TOKEN): Promise<void> {
  if (!token) fail("Cargo registry token is missing");
  ...
  if (!response.ok) { const detail = (await response.text()).replace(/\s+/gu, " ").trim().slice(0, 400); fail(`crates exact archive upload ${response.status}${detail ? `: ${detail}` : ""}`); }
}
async function createAttestation(artifactPath: string, artifactBytes: Uint8Array, environment: RuntimeEnvironment): Promise<string> {
  if (process.env.LENSO_RELEASE_MODE === "shadow") {
    ...
    if (!endpoint) fail("shadow attestation adapter is required");
    ...
    if (!response.ok) fail(`shadow attestation adapter ${response.status}`);
    const result = await response.json() as { url?: string };
    if (!result.url || !result.url.startsWith("https://")) fail("shadow attestation URL is invalid");
    return result.url;
  }
  const value = process.env.LENSO_PUBLISH_ATTESTATION_URL;
  if (!value) fail("official publish attestation URL is required");
...
type RecoveryAuthorization =
  | { kind: "production-break-glass"; authorizedRunUrl: string; authorizedRunSha256: Sha256; workflowCommit: string }
  | { kind: "production-partial"; failedRunUrl: string; workflowCommit: string; publishedPackages: PublishSelection[] }
  | { kind: "production-zero-write"; failedRunUrl: string; proofRunUrl: string; workflowCommit: string };

type ExpectedRecoveryKind = RecoveryAuthorization["kind"] | "production-publication";
...
export async function verifyRecoveryAuthorization(
  ...
  expectedKind: ExpectedRecoveryKind = "production-break-glass",
  ...
  const publicationRecovery =
    expectedKind === "production-publication" &&
    ["production-partial", "production-zero-write"].includes(
...
export async function authorizedRecoveryKind(environment: RuntimeEnvironment): Promise<RecoveryAuthorization["kind"]> {
  try { return (await verifyRecoveryAuthorization(environment, "production-publication")).kind; }
  catch (publicationError) {
    try { return (await verifyRecoveryAuthorization(environment, "production-break-glass")).kind; }
    catch (breakGlassError) { throw new AggregateError([publicationError, breakGlassError], "recovery authorization is invalid"); }
  }
}

export async function prepareAuthorizedRecovery(environment: RuntimeEnvironment): Promise<void> {
  const kind = await authorizedRecoveryKind(environment);
  if (kind === "production-partial" || kind === "production-zero-write") return preparePartialRecovery(environment);
  return prepareRecovery(environment);
}

export async function recoverAuthorized(environment: RuntimeEnvironment): Promise<ComponentReceiptV1[]> {
  const kind = await authorizedRecoveryKind(environment);
  if (kind === "production-partial" || kind === "production-zero-write") return recoverPartialPublished(environment);
...
async function partialRecoveryArtifacts(
  environment: RuntimeEnvironment,
  plan: ReleasePlanV1,
  publishedPackages: PublishSelection[],
  writeSubjects: boolean,
): Promise<Map<string, SealedArtifact>> {
  await stageCargoArchives(environment.cwd, plan, environment.packages);
  ...
  if (writeSubjects) await mkdir(subjectDirectory, { recursive: true, mode: 0o700 });
  for (const item of publicationOrder(plan, environment.packages)) {
    if (
      !item.id.startsWith("cargo:") &&
    ...
    if (observed.exists) {
      ...
      if (!matches) fail(`registry archive differs from reviewed artifact: ${item.id}`);
    ...
    const cargoMetadata = item.id.startsWith("cargo:")
      ...
      : null;
    const recovered: SealedArtifact = {
      path: artifact.path,
      bytes: subjectBytes,
      cargoMetadata,
      oci: artifact.oci ?? null,
    ...
    if (writeSubjects) {
      ...
      try { await handle.writeFile(subjectBytes); } finally { await handle.close(); }
    }
  }
  return artifacts;
}

export async function preparePartialRecovery(environment: RuntimeEnvironment): Promise<void> {
  const authorization = await verifyRecoveryAuthorization(
    environment,
    "production-publication",
  );
  const { candidateEnvironment, plan } = await recoveryPlan(
    environment,
    "production-publication",
  );
  const phases = publisherPackagePhases(candidateEnvironment.packages, plan, "recovery");
  for (const packages of phases) await preflight({ ...candidateEnvironment, packages });
  await partialRecoveryArtifacts(
    environment,
    plan,
    authorization.kind === "production-partial"
...
export async function recoverPartialPublished(environment: RuntimeEnvironment): Promise<ComponentReceiptV1[]> {
  const authorization = await verifyRecoveryAuthorization(
    environment,
    "production-publication",
  );
  const { plan } = await recoveryPlan(environment, "production-publication");
  const artifacts = await partialRecoveryArtifacts(
    environment,
    plan,
    authorization.kind === "production-partial"
      ? authorization.publishedPackages
  ...
  const receipts: ComponentReceiptV1[] = [];
  for (const item of publicationOrder(plan, environment.packages)) {
    const artifact = artifacts.get(`${item.id}\0${item.version}`); if (!artifact) fail("recovery artifact is missing");
    const name = item.id.slice(item.id.indexOf(":") + 1);
    ...
    const matches = item.id.startsWith("cargo:")
      ? cargoArchiveEquivalent(artifact.bytes, observed.bytes)
      : hash(artifact.bytes) === hash(observed.bytes);
    if (!matches) fail(`recovered registry archive differs from reviewed artifact: ${item.id}`);
    const receipt = receiptFor(plan, item, observed as Required<Omit<RegistryObservation, "exists">>, provenanceUrl, environment, basename(artifact.path), fixedGroup ? `${fixedGroup.name}@${fixedGroup.version}` : undefined);
    assertComponentReceipt(receipt);
    if (!fixedGroup) { await createImmutableTag(receipt, environment); await dispatchReceipt(receipt, environment); }
    receipts.push(receipt);
  }
  if (fixedGroup) {
    await createFixedGroupRelease(fixedGroup, receipts, artifacts, environment);
...
export function validateOfficialAttestationUrl(
  value: string,
  repository: string,
): string {
...
async function existingReceiptForObservation(item: PublishSelection, observed: RegistryObservation, planId: Sha256, environment: RuntimeEnvironment, fixedGroup?: { name: string; version: string }): Promise<ComponentReceiptV1 | null> {
  const recovered = await readExistingReceipt(item, environment, fixedGroup);
  if (!recovered) return null;
  if (!observed.bytes || !observed.integrity || !observed.url || !observed.publishedAt || recovered.planId !== planId || recovered.sourceCommit !== environment.releaseCommit || recovered.packedSha256 !== hash(observed.bytes) || recovered.registryIntegrity !== observed.integrity || recovered.registryUrl !== observed.url || recovered.publishedAt !== observed.publishedAt) fail("existing receipt contradicts authoritative registry state");
  return recovered;
}
...
export async function createPlan(cwd: string, repository: string, sourceCommit: string): Promise<ReleasePlanV1> {
  ...
  if (config.aliases && Object.entries(config.aliases).some(([target, source]) => !/^(?:artifact|oci):[a-z0-9-]+$/u.test(target) || !/^npm:@lenso\/[a-z0-9-]+$/u.test(source))) fail("repository component alias is invalid");
