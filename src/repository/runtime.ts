...
export async function publishSelected(environment: RuntimeEnvironment): Promise<ComponentReceiptV1[]> {
  ...
  for (const item of publicationOrder(plan, environment.packages)) {
    ...
    if (observed.exists) {
      const recovered = await matchingExistingReceipt(
        item,
        observed,
        plan,
        environment,
...
export async function recoverPartialPublished(environment: RuntimeEnvironment): Promise<ComponentReceiptV1[]> {
  ...
  for (const item of publicationOrder(plan, environment.packages)) {
    ...
    if (observed.exists) {
      ...
      if (recovered) {
        if (!fixedGroup) await dispatchReceipt(recovered, environment);
...
async function matchingExistingReceipt(
  item: PublishSelection,
  observed: RegistryObservation,
  plan: ReleasePlanV1,
  environment: RuntimeEnvironment,
  fixedGroup?: { name: string; version: string },
): Promise<ComponentReceiptV1 | null> {
  if (
    !observed.exists ||
    !observed.bytes ||
    !observed.integrity ||
  ...
  if (
    recovered.planId !== plan.planId ||
    recovered.sourceCommit !== environment.releaseCommit ||
    recovered.packedSha256 !== hash(observed.bytes) ||
    recovered.registryIntegrity !== observed.integrity ||
    recovered.registryUrl !== observed.url ||
    recovered.publishedAt !== observed.publishedAt
