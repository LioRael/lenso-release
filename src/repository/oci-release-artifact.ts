...
type Descriptor = { mediaType?: unknown; digest?: unknown; size?: unknown };
type JsonObject = Record<string, unknown>;

export type OciReleaseArtifact = {
  archiveBytes: Buffer;
...
function json(bytes: Uint8Array, label: string): JsonObject {
  try { return object(JSON.parse(Buffer.from(bytes).toString("utf8")), label); }
  catch (error) { throw new Error(`OCI release artifact: invalid ${label} JSON`, { cause: error }); }
}
function canonicalPath(name: string): string {
  const value = name.replace(/^\.\//u, "").replace(/\/$/u, "");
  if (!value || value.startsWith("/") || value.includes("\\") || value.split("/").some((part) => !part || part === "." || part === "..")) fail("archive contains an unsafe path");
  return value;
}
function octal(bytes: Buffer): number {
  const value = bytes.toString("ascii").replaceAll("\0", "").trim();
  if (!/^[0-7]+$/u.test(value)) fail("archive contains an invalid size");
  const parsed = Number.parseInt(value, 8);
  if (!Number.isSafeInteger(parsed) || parsed < 0) fail("archive entry size is invalid");
  return parsed;
}
function tarFiles(archive: Buffer): Map<string, Buffer> {
  ...
  while (offset + 512 <= archive.length) {
    ...
    const start = offset + 512; const end = start + size;
    if (end > archive.length) fail("archive entry is truncated");
    if (type === 0 || type === 48) {
      if (files.has(path)) fail(`archive contains duplicate path ${path}`);
      files.set(path, Buffer.from(archive.subarray(start, end)));
    } else if (type !== 53) fail(`archive entry type is forbidden: ${path}`);
    offset = start + Math.ceil(size / 512) * 512;
  }
  if (files.size === 0) fail("archive contains no files");
  return files;
}
function descriptor(value: unknown, label: string): { mediaType: string; digest: Sha256; size: number } {
  const raw = object(value, label);
  if (typeof raw.mediaType !== "string" || typeof raw.digest !== "string" || !DIGEST.test(raw.digest) || !Number.isSafeInteger(raw.size) || Number(raw.size) <= 0) fail(`${label} is invalid`);
...
export function inspectOciReleaseArtifact(input: {
  ...
  if (index.schemaVersion !== 2 || !Array.isArray(index.manifests) || index.manifests.length !== 1) fail("OCI index must select exactly one image manifest");
  const root = descriptor(index.manifests[0], "OCI index manifest descriptor");
  if (root.mediaType !== "application/vnd.oci.image.manifest.v1+json") fail("OCI index must directly select an image manifest");
  const blobs = new Map<Sha256, Buffer>();
  for (const [path, bytes] of files) {
    const match = /^blobs\/sha256\/([0-9a-f]{64})$/u.exec(path);
    if (!match) continue;
    const expected = `sha256:${match[1]}` as Sha256;
    if (digest(bytes) !== expected) fail(`blob digest mismatch: ${expected}`);
    blobs.set(expected, bytes);
  }
  const manifestBytes = blobs.get(root.digest) ?? fail("selected image manifest blob is missing");
  if (manifestBytes.length !== root.size) fail("selected image manifest size mismatch");
  const manifest = json(manifestBytes, "image manifest");
  if (manifest.schemaVersion !== 2 || !Array.isArray(manifest.layers)) fail("image manifest shape is invalid");
  const dependencies = [descriptor(manifest.config, "image config descriptor"), ...(manifest.layers as Descriptor[]).map((value, index) => descriptor(value, `image layer descriptor ${index}`))];
  for (const dependency of dependencies) {
    const bytes = blobs.get(dependency.digest) ?? fail(`referenced image blob is missing: ${dependency.digest}`);
    if (bytes.length !== dependency.size) fail(`referenced image blob size mismatch: ${dependency.digest}`);
  }
