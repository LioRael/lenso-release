...
import {
  authorizedRecoveryKind,
  consumePreflightProof,
  createPlan,
  createPreflightProof,
  prepareAuthorizedRecovery,
  preparePartialRecovery,
  publishSelected,
  recoverPartialPublished,
  recoverAuthorized,
...
if (command === "plan") {
  ...
  const authorization = await verifyRecoveryAuthorization(environment(), "production-publication") as { failedRunUrl: string };
  const runId = /\/actions\/runs\/([1-9][0-9]*)$/u.exec(authorization.failedRunUrl)?.[1];
  if (!runId) throw new Error("partial recovery failed run URL is invalid");
  process.stdout.write(`${runId}\n`);
} else {
  throw new Error(
    "usage: runtime plan|preflight|consume-preflight|publish|recover-kind|recover-prepare|recover|recover-partial-prepare|recover-partial|recover-partial-failed-run-id",
  );
}
