  ...
  try {
    ...
    try {
      ...
      if (manifestResponse.status === 401 && !options.token) {
        ...
        if (!match) return { failure: "http", detail: "OCI registry authentication challenge was invalid" };
        ...
        if (typeof tokenBody?.token !== "string") return { failure: "http", detail: `OCI registry token service returned HTTP ${tokenResponse.status}` };
