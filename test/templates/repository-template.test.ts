...
describe("repository template workflow contracts", () => {
  ...
  it("uses scoped App authentication and prioritizes fresh intent over a retained plan", async () => {
    ...
    expect(source).toContain("'.github/workflows/release-plan.yml'");
...
describe("publisher preflight execution gate", () => {
  ...
  it("accepts the exact reviewed runtime and rejects workflow, bundle and generated-file drift before publish", async () => {
    ...
    const identity = {
      ...
      packages: [{ id: "npm:@lenso/console-bridge", previousVersion: "0.1.0", nextVersion: "0.1.1", bump: "patch" as const, releaseGroup: "console", userFacing: true, dependencies: [] }],
    };
    ...
    try {
      ...
      await writeFile(join(fixture.cwd, ".lenso-release/config.json"), `${JSON.stringify({ schema: "lenso.repository-config.v1", repository: plan.repository, fixedGroups: { "lenso-cli": ["npm:@lenso/console-bridge", "cargo:lenso-cli"] } })}\n`);
  ...
  it("recovers an already-published npm archive and enqueues a deterministic receipt", async () => {
    ...
    const server = createServer((request, response) => { let body = ""; request.on("data", (chunk) => { body += chunk; }); request.on("end", () => { requests.push({ method: request.method, url: request.url, body }); response.setHeader("content-type", "application/json");
      ...
      else if (request.url?.startsWith("/registry/")) response.end(JSON.stringify({ time: { "0.1.1": "2026-07-11T00:00:00.000Z" }, versions: { "0.1.1": { dist: { integrity: `sha512-${createHash("sha512").update(archive).digest("base64")}`, tarball: "https://registry.npmjs.org/@lenso/console-bridge/-/console-bridge-0.1.1.tgz" } } } }));
      else if (request.url?.includes("/git/ref/tags/")) { if (tagMessage) response.end(JSON.stringify({ object: { sha: tagObjectSha, type: "tag" } })); else { response.statusCode = 404; response.end("{}"); } }
      else if (request.url?.endsWith(`/git/tags/${tagObjectSha}`)) response.end(JSON.stringify({ object: { sha: fixture.releaseCommit }, message: tagMessage }));
      else if (request.url?.endsWith("/git/tags")) { tagMessage = JSON.parse(body).message; response.end(JSON.stringify({ sha: tagObjectSha })); }
      else if (request.url?.endsWith("/git/refs")) response.end("{}");
      else if (request.url === "/preflight") { const bindingDigest = JSON.parse(body).bindingDigest; const issuedAt = new Date().toISOString(); proofIndex += 1; response.end(JSON.stringify({ schema: "lenso.publisher-preflight-proof.v1", proofId: `sha256:${proofIndex.toString(16).padStart(64, "0")}`, bindingDigest, issuedAt, expiresAt: new Date(Date.now() + 120_000).toISOString(), token: "signed-proof-token-signed-proof-token" })); }
    ...
    try {
      ...
      await createPreflightProof(environment); await consumePreflightProof(environment); const retried = await publishSelected(environment);
      expect(retried).toEqual(first); expect(requests.filter(({ method, url }) => method === "POST" && url?.endsWith("/git/tags"))).toHaveLength(1);
      expect(requests.filter(({ url }) => url === "/preflight")).toHaveLength(4); expect(requests.filter(({ url }) => url === "/consume")).toHaveLength(3); expect(requests.filter(({ url }) => url === "/receipt")).toHaveLength(2); expect(requests.filter(({ url }) => url === "/cleanup")).toHaveLength(0);
    } finally { server.close(); for (const [key, value] of Object.entries({ PATH: saved.PATH, RUNNER_IMAGE: saved.RUNNER_IMAGE, LENSO_NPM_REGISTRY_URL: saved.npm, LENSO_TEST_ARTIFACT_PROXY_URL: saved.proxy, LENSO_GITHUB_API_URL: saved.github, LENSO_COORDINATOR_PREFLIGHT_URL: saved.preflight, LENSO_COORDINATOR_PREFLIGHT_CONSUME_URL: saved.consume, LENSO_PREFLIGHT_AUTHORITY_PUBLIC_KEY: saved.publicKey, LENSO_COORDINATOR_RECEIPT_URL: saved.receipt, LENSO_COORDINATOR_CLEANUP_URL: saved.cleanup, LENSO_APP_ID: saved.app, LENSO_PUBLISH_ATTESTATION_URL: saved.attestation })) value === undefined ? delete process.env[key] : process.env[key] = value; }
  });
});
